...
export default function AdminEditBlogsPage() {
	...
	const handleSave = async (statusOverride?: "draft" | "published") => {
		...
		if (editingBlog.id) {
			...
			await actualizarBlog(editingBlog.id, payload);
			saved = { ...editingBlog, ...payload } as Blog;
		} else {
			// Crear nuevo blog
			const newId = await crearBlog(payload as Blog);
			saved = { ...payload, id: newId } as Blog;
		}
	...
	const handleMakeFeatured = async () => {
		...
		try {
			if (editingBlog.featured) {
				// Quitar destacado
				await actualizarBlog(editingBlog.id, { featured: false });
				setBlogs((prev) =>
				...
				await actualizarBlog(editingBlog.id, { featured: true });
				setBlogs((prev) =>
